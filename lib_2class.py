...
def visualizeSinglemodelc(model, data, labels,typeofdata):
    ...
    transformed_data = model.fit_transform(data,labels,scale=True)
    #transformed_data = pipeline[1].transform(transformed_data1,scale=True)

    #transformed_data = model.transform(transformed_data1, True)
    ...
    #transformed_model = model.transform(pipeline[1].prototypes_,scale=True)
    transformed_model = model.transform(model.prototypes_, scale = True)
    #transformed_model = model.prototypes_
